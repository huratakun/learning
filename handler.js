        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit anda habis, silahkan beli melalui *.buy*`, m)
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (!text) continue
        this.sendHydrated(id, text, wm + '\n\n' + botdate, hwaifu.getRandom(), sgc, 'Group', null, null, [
      ['🎀 Menu', '/menu'],
      ['🪄 Test', '/ping']
...
if (msgg) return conn.sendButton(m.chat, `${global.htki} DAFTAR ${global.htka}`, msgg, null, ['DAFTAR', '/daftar'],m)
}
