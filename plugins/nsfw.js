...
conn.sendButton(m.chat, `Nihh ${command}`.trim(), author, haha.data, [['🔄 Next 🔄', `/${command}`]], m)}
if (command == 'hentai3') {
let haha = await conn.getFile(`https://api.xteam.xyz/randomimage/hentai3?APIKEY=HIRO`)
conn.sendButton(m.chat, `Nihh ${command}`.trim(), author, haha.data, [['🔄 Next 🔄', `/${command}`]], m)}
if (command == 'nsfworgy') {
...
handler.command =  handler.help = ['nsfwloli', 'nsfwfoot', 'nsfwass', 'nsfwbdsm', 'nsfwcum', 'nsfwero', 'nsfwfemdom', 'nsfwfoot', 'nsfwglss', 'nsfworgy', 'yuri', 'yuri2', 'yaoi', 'yaoi2', 'panties', 'tetas', 'booty', 'ecchi', 'furro', 'hentai3', 'trapito', 'imagenlesbians', 'pene', 'porno', 'porno2', 'randomxxx', 'pechos']
handler.tags = ['nsfw']
